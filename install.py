...
def infra_agent_install(config, license_key):
    ...
    deb_agent_url = config['front']['base_url'] + config['front']['deb_agent_path']
    rpm_agent_url = config['front']['base_url'] + config['front']['rpm_agent_path']

    print("")
    print("=== INFRA Agent 다운로드 ===")
    temp_base_dir = input(f"INFRA Agent 다운로드할 경로를 입력하세요 (default : {default_base_dir}) : ")
    # 기존 경로입력확인 if문 로직을 or로 바꿈
    temp_base_dir = temp_base_dir.strip() or default_base_dir
    ...
    if path_permission["permission"] != "read-write":
        ...
        sys.exit(1)  # 권한 부족 강제종료

    if os.path.exists("/etc/debian_version"):
        print("감지된 OS: ubuntu Linux")
        package_type = "deb"
        agent_url = deb_agent_url
    else:
        print("감지된 OS: redhat Linux")
        package_type = "rpm"
        agent_url = rpm_agent_url
        
    os.makedirs(temp_base_dir, exist_ok=True)
    agent_file_path = os.path.join(temp_base_dir, os.path.basename(agent_url))

    download_file_with_progress(agent_url, agent_file_path)
    print("")
    print("=== INFRA Agent 설치 ===")
    install_package(agent_file_path, package_type)
